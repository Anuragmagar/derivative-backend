...
class sHTMLPrinter(LaTeXPrinter):
    def __init__(self):
        super(sHTMLPrinter, self).__init__()
        self.lines = ['<ol>']

    def format_math(self, math):
        return '<StaticMathField>{}</StaticMathField>'.format(
            latex(math))
...
class HTMLPrinter(DiffPrinter, sHTMLPrinter):
    def __init__(self, rule):
        self.alternative_functions_printed = set()
        sHTMLPrinter.__init__(self)
    ...
    def finalize(self):
        ...
        self.level = 0
        self.append('<div id="ans">')
        self.append('<div id="output-head"><div><span className="font-light text-xs">FIRST DERIVATIVE</span><StaticMathField>{}</StaticMathField></div></div>')
...
@app.route("/api/v1/derivative/", methods=['POST'])
def hello_world():
    qn = request.get_json()
    integrationLatex = qn['derivativeLatex']
    
    symbol = sympy.Symbol(qn['variable'])
    latex_string = fr"{integrationLatex}"
    equation = parse_latex(latex_string)
    
    derivative = sympy.diff(equation, symbol)
    if qn['withStep']:
        a = HTMLPrinter(diff_steps(equation, symbol))
        data = a.finalize()
        return [
            '{}'.format(rcode(derivative)),
            data,
            '{}'.format(rcode(equation)),
            # jsonify(data)
        ]
    ...
    return [
        '{}'.format(rcode(derivative)),
        '<div id="ans"><div id="output-head"><div><span className="font-light text-xs">FIRST DERIVATIVE</span></div></div><p>The answer is:</p><p><StaticMathField>{}</StaticMathField></p></div>'.format(latex(derivative)),
        '{}'.format(rcode(equation)),
    ]
